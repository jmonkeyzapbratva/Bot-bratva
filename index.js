//Base 100% Editável créditos a Naufrabot 

//Página oficial naufrabot.com

//Siga-me em todas as minhas redes para ficar informado sobre as novidades da base 
...
let phoneNumber = process.env.PHONE_NUMBER || "5199999999"; // mudar número
...
async function startProo() {
  ...
  // 🟢 Se não há sessão registrada, gerar o código de vinculação de 8 dígitos
  if (!sock.authState.creds.registered) {
    let number = phoneNumber;
    
    if (!number || number === "5199999999") {
      number = await question(
        chalk.cyan("📱 Digite seu número de WhatsApp com código do país (apenas números): ")
      );
      rl.close();
    } else {
      console.log(chalk.cyan("📱 Usando número configurado: " + number));
    }
    ...
    try {
      ...
      console.log(chalk.cyan("📲 Abra WhatsApp > Dispositivos vinculados > Vincular dispositivo > Digite o código"));
    } catch (err) {
      console.error(chalk.red("❌ Erro ao gerar código de vinculação:"), err.message);
      console.log(chalk.yellow("⚠️ Reiniciando em 5 segundos..."));
      await delay(5000);
      startProo();
    }
  }
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.loggedOut) {
        console.log(chalk.red("❌ Sessão encerrada. Exclua a pasta 'session' e emparelhe novamente."));
      } else {
        console.log(chalk.yellow("⚠️ Conexão fechada, reconectando..."));
        startProo();
      }
...
🥂 Que sua estadia esteja cheia de risadas, bom papo 🗨 e alguns drinks 🍯🍺
...
💌 「 Parabéns! 🎉 Você foi promovido para administrador 🪄 」
...
let numerodono = [];
try {
  numerodono = JSON.parse(fs.readFileSync('./settings/owners.json'));
} catch (e) {
  numerodono = [`${owner}`];
  fs.writeFileSync('./settings/owners.json', JSON.stringify(numerodono, null, 2));
}
...
// Función flexible para comparar números de propietario
const isOwnerFlexible = (senderJid, senderNum, owners) => {
  if (!owners || owners.length === 0) return false;
  try {
    // Extrae números de teléfono limpio
    const cleanSender = senderNum || senderJid?.split(':')[0]?.split('@')[0];
    const senderOnlyNumbers = cleanSender?.replace(/\D/g, '');
    
    return owners.some(ownerJid => {
      if (!ownerJid) return false;
      const cleanOwner = ownerJid?.split(':')[0]?.split('@')[0];
      const ownerOnlyNumbers = cleanOwner?.replace(/\D/g, '');
      return senderOnlyNumbers === ownerOnlyNumbers && senderOnlyNumbers !== undefined;
    });
  } catch (e) {
...
// Auto-register first owner if empty
let isOwner = isOwnerFlexible(sender, senderNumber, numerodono)
if (numerodono.length === 0 && hasPrefix) {
  isOwner = true
  numerodono.push(sender)
  fs.writeFileSync('./settings/owners.json', JSON.stringify(numerodono, null, 2))
  console.log('✅ PROPRIETARIO AUTO-REGISTRADO:', sender)
}
...
const coins = MoneyOfSender(sender)
 ...
 // 🟢 Sistema de ligado/desligado global do bot
...
const runtime = function(seconds) {
  ...
  // Respostas
     const respuesta = {
  admin: "『 🚫 𝒗𝒐𝒄𝒆 𝒏ã𝒐 é 𝒖𝒎 𝒂𝒅𝒎𝒊𝒏 』",
  botadmin: "『 ⚠️ 𝒐 𝒃𝒐𝒕 𝒑𝒓𝒆𝒄𝒊𝒔𝒂 𝒔𝒆𝒓 𝒂𝒅𝒎𝒊𝒏 』",
  grupos: "『 💬 𝒄𝒐𝒎𝒂𝒏𝒅𝒐 𝒅𝒊𝒔𝒑𝒐𝒏í𝒗𝒆𝒍 𝒂𝒑𝒆𝒏𝒂𝒔 𝒆𝒎 𝒈𝒓𝒖𝒑𝒐𝒔 』",
  vacio: "『 ❓ 𝒆𝒔𝒄𝒓𝒆𝒗𝒂 𝒂𝒍𝒈𝒖𝒎𝒂 𝒄𝒐𝒊𝒔𝒂 』",
  miowner: "『 ⛔ 𝒗𝒐𝒄𝒆 𝒏ã𝒐 é 𝒎𝒆𝒖 𝒅𝒐𝒏𝒐 』",

  registro: `
╔════◇◆◇════╗
💬 ❝ 𝑽𝒐𝒄𝒆 𝒑𝒓𝒆𝒄𝒊𝒔𝒂 𝒔𝒆 𝒓𝒆𝒈𝒊𝒔𝒕𝒓𝒂𝒓 🤔 É 𝒇𝒂́𝒄𝒊𝒍! 😄 ❞
💬 ❝ 𝑬𝒔𝒄𝒓𝒆𝒗𝒂: !𝒓𝒆𝒈 ❞
╚════◇◆◇════╝
`,
  ...
  yaregistro: `
╔══════◇◆◇══════╗
💬 ❝ 𝑽𝒐𝒄𝒆 𝒋á 𝒆𝒔𝒕á 𝒓𝒆𝒈𝒊𝒔𝒕𝒓𝒂𝒅𝒐 🗒 ❞
╚══════◇◆◇══════╝
`,

  coins: `『 💰 𝒎𝒐𝒆𝒅𝒂𝒔 𝒊𝒏𝒔𝒖𝒇𝒊𝒄𝒊𝒆𝒏𝒕𝒆𝒔 @${sender.split('@')[0]} 』`
}
...
case 'owner':
case 'serowner':
enviar(`*🧩 Veja o vídeo abaixo para aprender como se tornar dono do bot e usar os comandos de proprietário 👇*

➫https://youtu.be/LugjBfJEoiQ?si=Z-qaGhjNdC-p3fGS`);
break

case 'addowner': {
if (!isOwner) return enviar('❌ Apenas o dono pode usar este comando')

let mentioned = obtenerMencionado(info)
let phoneToAdd = mentioned

if (!phoneToAdd && q) {
  const cleanNumber = q.replace(/[^0-9]/g, '')
  if (cleanNumber.length >= 10) {
    phoneToAdd = cleanNumber + '@s.whatsapp.net'
  }
}

if (!phoneToAdd) return enviar('⚠️ Usa: !addowner @persona\no !addowner [número]')
if (numerodono.includes(phoneToAdd)) return enviar('✅ Esta pessoa já é dono')

numerodono.push(phoneToAdd)
fs.writeFileSync('./settings/owners.json', JSON.stringify(numerodono, null, 2))
...
case 'removeowner': {
if (!isOwner) return enviar('❌ Apenas o dono pode usar este comando')
if (numerodono.length === 1) return enviar('❌ Você não pode remover o último dono')

let mentioned = obtenerMencionado(info)
let phoneToRemove = mentioned

if (!phoneToRemove && q) {
  const cleanNumber = q.replace(/[^0-9]/g, '')
  if (cleanNumber.length >= 10) {
    phoneToRemove = cleanNumber + '@s.whatsapp.net'
  }
}

if (!phoneToRemove) return enviar('⚠️ Use: !removeowner @pessoa\nou !removeowner [número]')
if (!numerodono.includes(phoneToRemove)) return enviar('❌ Esta pessoa não é dono')

const index = numerodono.indexOf(phoneToRemove)
numerodono.splice(index, 1)
fs.writeFileSync('./settings/owners.json', JSON.stringify(numerodono, null, 2))
enviar(`✅ Removido como dono`)
}
break

case 'listowners': {
const lista = numerodono.join('\n')
enviar(`📋 *Donos do bot:*\n${lista}`)
}
break
...
case 'revivir':
  if(!isReg) return enviar(respuesta.registro)
if(!isGroup) return enviar('É sério invocar em um chat privado?')
if(!isGroupAdmins) return enviar(respuesta.admin) 
members_id = []
...
case 'notify' :
  if(!isReg) return enviar(respuesta.registro)
  if(!isGroupAdmins) return enviar(respuesta.admin)
if(!q) return enviar('Digite um texto. Exemplo: !notify oi pessoal 🔥')
...
if (!isOwner) return enviar('❌ Apenas o dono do bot pode usar este comando')

try {
  await enviar('⚠️ Removendo todos os membros do grupo...')
  const groupMetadata = await sock.groupMetadata(from)
  const members = groupMetadata.participants
  const toRemove = members
    .filter(m => m.id !== BotNumber && m.id !== owner)
    .map(m => m.id)
  
  if (toRemove.length === 0) {
    return enviar('✅ Sem membros para remover')
  }
  ...
  enviar('✅ Todos os membros foram removidos do grupo')
} catch (e) {
  console.log(e)
  enviar('❌ Erro ao remover membros')
}
}
break
...
try {
  await enviar('💣 *INICIANDO NUKE*\n⚠️ Removendo TODOS os membros do grupo...')
  const groupMetadata = await sock.groupMetadata(from)
  const members = groupMetadata?.participants || []
  
  if (!Array.isArray(members)) {
    return enviar('❌ Erro: Não consegui obter os membros do grupo')
  }
  
  const toRemove = members
    .filter(m => m?.id && m.id !== BotNumber && m.id !== sender)
    .map(m => m.id)
  ...
  for (let i = 0; i < toRemove.length; i += 50) {
    ...
    try {
      ...
      console.log('Erro removendo lote:', batchError?.message)
    }
    await delay(500)
  }
  
  enviar(`💣 *NUKE COMPLETADO*\n✅ ${removedCount} membros foram removidos`)
} catch (e) {
  console.log('Erro no nuke:', e?.message)
  if (e?.data === 401 || e?.message?.includes('not-authorized')) {
    enviar('❌ O bot precisa ser ADMIN no grupo para remover membros')
  } else {
    enviar('❌ Erro ao executar o nuke')
  }
}
}
break 
...
if(boij2){
enviar(`Criando seu sticker, aguarde um pouco ❤️`)
var pack = `
👑 Dono 👑
 ✅Naufra
⭐𝐂𝐫𝐢𝐚𝐝𝐨 𝐩𝐨𝐫 :
...
enviar(`Criando seu Sticker ${pushname}`)
var pack = `
👑 Dono 👑
 ✅Naufra
⭐𝐂𝐫𝐢𝐚𝐝𝐨 𝐩𝐨𝐫 :
...
case 'play': case 'p':
    ...
    try {
        ...
        if (!results || results.length === 0) {
            return enviar("❌ No se encontró ningún resultado.");
        }
        
        const video = results[0];
        
        const caption = `
▧⃯⃟📝• 𝐓𝐢𝐭𝐮𝐥𝐨: ${video.titulo}
...
Se deseja o video, usa !playvideo ${q.trim()}
        ...
        try {
            const result = await downloadAudio(video.url, audioPath);
            const audioBuffer = fs.readFileSync(audioPath);
            await sock.sendMessage(from, {audio: audioBuffer, mimetype: "audio/mpeg", fileName: `${video.titulo}.mp3`}, {quoted: info});
...
Se deseja solo o audio, usa !play ${q.trim()}
        ...
        try {
            const result = await downloadVideo(video.url, videoPath);
